...
function Header() {
	...
	const Completionist = () => <div className="hdFSNp depositCountDown text-primary">...</div>;
	const connectWallet = () =>{
	...
	const renderer = ({ days,hours, minutes, seconds, completed }) => {
		if (completed) {
		  ...
		  // Render a countdown
		  return <div className="hdFSNp depositCountDown text-primary">{days}:D {hours}:H {minutes}:M {seconds}:S</div>
		}
	  };
    ...
    <header className="header Huans flexC">
		...
		<div className="hdFS flexC fl-bet">
			<div className="hdFSN flexC">
				...
				&nbsp;
				<div className="hdFSNp contractAddress"><a href={`https://testnet.bscscan.com/address/${financeAppContractAddress}`} className="stack_p" target="_blank" >{(financeAppContractAddress?.substring(0, 8) + "..." + financeAppContractAddress?.substring(financeAppContractAddress?.length - 8))}</a></div>
			</div>
			<div className="hdFSN flexC">
				<img src={platform} />
				
				<div className="hdFSNp runTime text-light">Platform Running time: <span className='text-primary'>{depositTime}</span> </div>
			</div>
			<div className="hdFSN flexC">
				<img src={income}/>
				<p>
